...
// listBlocksWith scans content for all complete sentinel-delimited blocks keyed
// by beginPrefix/endPrefix and returns them in file order. Incomplete blocks
// (BEGIN with no matching END) are silently skipped. CRLF is normalised to LF
// before scanning so Windows-synced configs parse correctly. This is the
// parameterized core shared by ListBlocks (managed) and ListProvisionalBlocks.
func listBlocksWith(content []byte, beginPrefix, endPrefix string) []NamedBlock {
...
// removeBlockWith returns content with the sentinel-delimited block for name
// removed. Parameterized core shared by RemoveBlock and RemoveProvisionalBlock.
...
// — is preserved byte-for-byte (WR-04).
func removeBlockWith(content []byte, name, beginPrefix, endPrefix string) []byte {
	...
	// (WR-04). removeBlockWith never consumes a line it does not own.
	afterEnd := endIdx + 1
...
// replaceBlockWith returns existing with the sentinel-delimited block for name
// set to blockBody. Parameterized core shared by ReplaceBlock and
// ReplaceProvisionalBlock. If a block for name already exists, only the lines
// from its BEGIN marker through its END marker (inclusive) are replaced; every
// other line is byte-identical before and after. If no such block exists, a new
// block is appended after all existing content.
func replaceBlockWith(existing []byte, name, blockBody, beginPrefix, endPrefix string) []byte {
	beginMarker := beginPrefix + name
	endMarker := endPrefix + name
	...
	for i, line := range lines {
		// Trim trailing \r as well as \n so CRLF-encoded markers (Windows-synced
		// configs) still match an existing block — otherwise replaceBlockWith would
...
func ListBlocks(content []byte) []NamedBlock {
	return listBlocksWith(content, BeginPrefix, EndPrefix)
}
...
func RemoveBlock(content []byte, name string) []byte {
	return removeBlockWith(content, name, BeginPrefix, EndPrefix)
}
...
func ReplaceBlock(existing []byte, name, blockBody string) []byte {
	return replaceBlockWith(existing, name, blockBody, BeginPrefix, EndPrefix)
}
