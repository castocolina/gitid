...
// marker, D-02). Every reserved wiring block (IsReservedBlockName — the gitid
// `Host *` globals stanza under BOTH registered sentinel names AND the
// gitid-owned `ssh-include` Include line) is skipped: none is a Host block
// with identity-shaped directives, and identity.Reconstruct's caller
// (internal/identity/loader.go) has no other chokepoint to exclude them, so a
// fresh D-06 machine's very first create would otherwise surface a phantom
// "ssh-include" row in the Identity Manager the moment the Include line is
// written.
