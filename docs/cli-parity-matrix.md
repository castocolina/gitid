...
|---|---|---|---|---|---|
| KEY-01, SSHUI-01, TEST-01, SHELL-03 | Create a new identity | `gitid identity create` / `gitid create` | `--name <n> --provider <p> --git-name <n> --git-email <e> [--ssh-host <alias> --hostname <h> --port <p> --algorithm <algo> --reuse-key <path> --strategy <gitdir|hasconfig|both> --git-dir <dir> --force-ssh --no-upload] --yes` | shipped | D-02 adaptive depth: missing a required flag opens the pre-filled wizard on a terminal, or errors naming the missing flags off one. `--dry-run` runs both gate stages and prints the four artifact previews, plus (09-05-PLAN.md Task 3) the upload command preview and the frozen dry-run note. `--force-ssh` (WR-06, default off) opts into the machine-global provider URL rewrite; the TUI exposes the same toggle. `--no-upload` suppresses the derived autonomous provider-key registration this verb otherwise attempts by default (same eligibility derivation as the wizard's checkbox) and prints the manual instructions instead; upload never changes this verb's exit code either way (D-03/D-11). |
...
| MGR-04, SHELL-03 | Clone an identity reusing the source key | `gitid identity clone <source>` / `gitid clone <source>` | `--name <n> [--yes --no-upload]` | shipped | D-15: clone has no second write pipeline — the re-derived values feed the create lifecycle. `--dry-run` runs both gate stages (D-16 full gate even for a same-key clone) and prints the previews, plus the upload command preview and the frozen dry-run note. The clone's provider URL rewrite (WR-06) mirrors the SOURCE's own current setting — there is no `--force-ssh` flag on this verb. `--no-upload` suppresses the derived autonomous upload and prints the manual instructions instead; upload never changes this verb's exit code either way (D-03/D-11). |
| MGR-04, SHELL-03 | Clone an identity with a fresh key | `gitid identity clone <source>` / `gitid clone <source>` | `--name <n> --new-key [--yes --no-upload]` | shipped | `--new-key` generates at the new canonical path instead of reusing the source's key. |
| MGR-05, KEY-07, SHELL-03 | New key for an existing identity (repair) | `gitid identity new-key <name>` / `gitid new-key <name>` | `[--yes --no-upload]` | shipped | Calls the SAME `runRepair` the TUI's CommitNewKey calls; repair never touches pre-existing key material. `--dry-run` tests the CURRENT key and creates nothing, plus (09-05-PLAN.md Task 3) previews an upload of that current key. `--no-upload` suppresses the derived autonomous upload and prints the manual instructions instead; upload never changes this verb's exit code either way (D-03/D-11). |
| KEY-05, SHELL-03 | Rotate an identity's key | `gitid identity rotate <name>` / `gitid rotate <name>` | `[--yes --no-upload]` | shipped | Calls the SAME `runRotate` the TUI's CommitRotate calls; archives the old pair, generates the new one at the same canonical paths. `--dry-run` labels the CURRENT key and carries the frozen post-rotation caveat, plus previews an upload of that current key. `--no-upload` suppresses the derived autonomous upload and prints the manual instructions instead; upload never changes this verb's exit code either way (D-03/D-11). |
| UP-01, UP-02, UP-03, SHELL-03 | Register an identity's public key with its provider (authentication + signing) | `gitid identity register-key <name>` / `gitid register-key <name>` | `[--dry-run]` | shipped | Calls the SAME `runUploadFor` the TUI's upload section calls (09-05-PLAN.md Task 1). This is the manual re-run surface for the autonomous registration plan 09-04 builds into the wizard's checkbox — 09-CONTEXT.md's `gitid copy --upload-keys` phrasing predates the archived `copy` command's removal; see 09-05-PLAN.md's objective for the recorded, binding deviation. Exit contract (R10, deliberately DIFFERENT from the four write verbs below): exit 0 on any success, partial success, already-complete, an ineligible provider (manual instructions printed), or `--dry-run`; non-zero ONLY when at least one registration was attempted and every attempted registration failed — registration is this verb's own primary operation, so D-03 requires reporting its failure rather than masking it. `--dry-run` performs no remote MUTATION; the auth-status check and the inventory read used to compute the missing-type diff (R11) may still run as read-only probes. A missing name on both TTYs opens the Identity Manager; any other descriptor combination exits 1 naming the identity argument. |
...
| HLTH-01, HLTH-03, HLTH-06, SHELL-03 | View health findings | `gitid health` | `[--json]` | shipped | Reads are always headless. The frozen `gitid.health/v1` envelope carries parse-error-suppressed findings. Exit status is tiered: 0 none, 1 info/warning, 2 error, 3 critical. |
| HLTH-05, SHELL-03 | View one identity's health findings | `gitid health` | `--identity <name> [--json]` | shipped | Matches the Identity Manager's `h` deep-link: only the named identity's own SSH/Git findings are included; global findings are excluded. |
| HLTH-04, FIX-01, SHELL-03 | Apply one suggested health fix | `gitid fix` | `[--dry-run]` | shipped | Applies the next fixable finding through the same D-10 verification loop as the TUI. Without `--yes`, every Fn-based fix asks for confirmation. |
| FIX-01, SHELL-03 | Apply all fixable health findings | `gitid fix` | `--yes` | shipped | Re-runs the full scan after each applied fix (D-13), never skips the mandatory verification loop, and halts after the bounded convergence backstop. |
| FIX-01, SHELL-03 | Preview health fixes without writing | `gitid fix` | `--dry-run` | shipped | Prints every fixable finding's real diff and writes nothing. |
| HLTH-05, FIX-01, SHELL-03 | Use the hidden Docker-style health compatibility alias | `gitid doctor` | `[--identity <name>] [--json] [--fix [--yes] [--dry-run]]` | shipped | Hidden from help. Plain `doctor` shares `health` exactly; `doctor --fix` shares `fix` exactly. `--fix` alone remains interactive, and only `--yes` removes per-finding confirmation. |
...
|---|---|---|
| create, clone | run stage 1 and stage 2 against the staged key, print outcomes and the four artifact previews; print the upload command preview(s) and the frozen dry-run note (unless `--no-upload`) — may perform the auth-status check and the key-inventory read (R11) | write to `~/.ssh` or `~/.gitconfig`; the staging directory is cleaned up; run `ssh-key add` or any other remote MUTATION |
| rotate, new-key | print the ceremony plan (targets, archive destination shape, provider host); run the connectivity test against the identity's CURRENT key when it exists; print the upload command preview(s) for that CURRENT key and the frozen dry-run note (unless `--no-upload`) — may perform the auth-status check and the key-inventory read (R11) | generate a key, stage anything, archive, or write; run `ssh-key add` or any other remote MUTATION |
| register-key | print the exact command(s) about to run and the frozen dry-run note; may perform the auth-status check and the key-inventory read (R11 — used to compute the missing-type diff) | run `ssh-key add` or any other remote MUTATION |
