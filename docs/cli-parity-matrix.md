...
| HLTH-01, HLTH-03, HLTH-06 | List health findings (all 9 check families) | `gitid health` | `[--json]` | shipped | Reads are always headless. Calls the SAME `doctor.Run(buildDoctorDeps(home))` construction the TUI's Health/Fixer tabs consume (one source, three consumers, 08-01-PLAN.md). The `--json` shape is PROVISIONAL — a flat array, not yet the frozen versioned-envelope convention the other `--json` rows use; a later Phase 8 wave supersedes it and updates this row's Notes. |
| HLTH-04, FIX-01 | Apply suggested health fixes | `gitid fix` | `[--yes] [--dry-run]` | shipped | Applies fixable findings one at a time, re-running the full scan (D-13) after each apply before considering the next — capped by a `maxPasses` hard backstop (D-14/T-08-06). Prefers `Fix.Interactive` over `Fix.Fn` when a finding's fix sets it (the SAME cmd-layer apply-gate precedence `doctor.go`'s `FixDescriptor` documents); `--yes` skips the per-finding confirm on an `Fn`-based fix and threads `assumeYes` into an `Interactive` one. `--dry-run` previews every fixable finding's real diff and writes nothing. |
