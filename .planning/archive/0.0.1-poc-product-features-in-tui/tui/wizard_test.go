...
func TestWizardFormEmailValidation(t *testing.T) {
	deps := fakeWriteTUIDeps(nil)
	w := makeTestWizardModel(deps)
	// Screen 3 (Git config) validates the email on confirm (Enter on last field).
	...
	w.focusIdx = screen3FocusCount - 1      // last Screen-3 field: Enter triggers confirm

	w2, cmd := w.handleKey(tea.KeyPressMsg{Code: tea.KeyEnter})

	if cmd != nil {
		t.Error("invalid email must NOT dispatch the LEG-2 write")
	}
	if w2.step != wizardStepForm {
	...
	if w2.focusIdx != 1 {
		t.Errorf("focus must move to the Git Email field (Screen-3 position 1); got %d", w2.focusIdx)
	}
}
...
// TestWizardScreen3ShowsGitConfigFields verifies Screen 3 (screenGitConfig, LEG 2)
// renders the git-side fields (Git Name / Git Email / Match Strategy / Signing) and
// a live includeIf preview, and does NOT render the SSH-only rows (collected on
// Screens 1–2). Requirement: TUI-04, GIT-02.
func TestWizardScreen3ShowsGitConfigFields(t *testing.T) {
	w := makeTestWizardModel(fakeWriteTUIDeps(nil))
	w.inputs[0].SetValue("personal")
	w.screen = screenGitConfig
	w.step = wizardStepForm
	w.focusIdx = 0

	view := w.view(90)

	for _, want := range []string{"Git Configuration", "Git Name", "Git Email", "Match Strategy", "Signing", "includeIf"} {
		if !strings.Contains(view, want) {
			t.Errorf("Screen 3 must show %q; got:\n%s", want, view)
		}
	}
	for _, notWant := range []string{"Hostname:", "SSH Alias:", "Port:"} {
		if strings.Contains(view, notWant) {
			t.Errorf("Screen 3 must NOT show SSH-only field %q; got:\n%s", notWant, view)
		}
	}
}

// TestWizardScreen3WritesLeg2OnConfirm drives the real model path: on Screen 3
// with a valid email, Enter on the last field writes LEG 2 via PersistGitconfig
// (fragment + includeIf + allowed_signers) and advances to Screen 4 — WITHOUT
// re-writing the SSH config. Requirement: GIT-02, SIGN-01, D-10 (CLI↔TUI parity).
func TestWizardScreen3WritesLeg2OnConfirm(t *testing.T) {
	var wroteGitconfig, wroteFragment, wroteSigners, wroteSSH bool
	...
	deps.identity.WriteFragment = func(_, _, _, _ string, _ bool) error {
		wroteFragment = true
		return nil
	}
	deps.identity.WriteAllowedSigners = func(_, _, _ string) (string, error) {
		wroteSigners = true
		return "sign.bak", nil
	}
	deps.identity.WriteSSH = func(_, _, _ string) (string, error) {
	...
	w := makeTestWizardModel(deps)
	w.inputs[0].SetValue("personal")
	w.inputs[1].SetValue("Pedro Perez")
	w.inputs[2].SetValue("pedro@example.com")
	w.inputs[3].SetValue("github.com")
	w.staged = identity.StagedKey{
		FinalPrivatePath: "/home/u/.ssh/id_ed25519_personal",
		FinalPubPath:     "/home/u/.ssh/id_ed25519_personal.pub",
		PubLine:          "ssh-ed25519 AAAA personal@gitid",
	}
	w.screen = screenGitConfig
	w.step = wizardStepForm
	w.focusIdx = screen3FocusCount - 1 // last field (Signing) — Enter writes LEG 2

	w2, cmd := w.handleKey(tea.KeyPressMsg{Code: tea.KeyEnter})
	if w2.err != "" {
		t.Fatalf("valid email must not error on confirm; got %q", w2.err)
	}
	if cmd == nil {
		t.Fatal("Enter on the last Screen-3 field must dispatch the LEG-2 write")
	}
	msg := cmd()
	rm, ok := msg.(wizardLeg2ResultMsg)
	if !ok {
		t.Fatalf("LEG-2 write returned %T, want wizardLeg2ResultMsg", msg)
	}
	if rm.err != nil {
		t.Fatalf("LEG-2 write errored: %v", rm.err)
	}
	if !wroteGitconfig || !wroteFragment || !wroteSigners {
		t.Errorf("LEG-2 must write includeIf(%v) + fragment(%v) + allowed_signers(%v)", wroteGitconfig, wroteFragment, wroteSigners)
	}
	if wroteSSH {
		t.Error("LEG-2 must NOT re-write the SSH config (WriteSSH called)")
	}

	// Feeding the result through update advances to Screen 4 (review).
	w3, _ := w2.update(rm)
	if w3.screen != screenReview {
		t.Errorf("after LEG-2 write, screen = %v, want screenReview (Screen 4)", w3.screen)
	}
}

// TestWizardScreen3EmailValidation verifies a malformed Git Email is caught on
// Screen 3 (before any write), not deep in the fragment write. Requirement: GIT-02.
func TestWizardScreen3EmailValidation(t *testing.T) {
	w := makeTestWizardModel(fakeWriteTUIDeps(nil))
	w.inputs[0].SetValue("personal")
	w.inputs[2].SetValue("not-an-email")
	w.screen = screenGitConfig
	w.step = wizardStepForm
	w.focusIdx = screen3FocusCount - 1

	w2, cmd := w.handleKey(tea.KeyPressMsg{Code: tea.KeyEnter})
	if w2.err == "" {
		t.Error("malformed email must set an error on Screen 3 confirm")
	}
	if cmd != nil {
		t.Error("malformed email must NOT dispatch the LEG-2 write")
	}
}

// TestWizardScreen4ReviewSummarizesArtifacts verifies Screen 4 (read-only review)
// renders the written SSH Host block, the gitconfig includeIf, the fragment, and
// the allowed_signers line. Requirement: TUI-04 (trustworthy final summary).
func TestWizardScreen4ReviewSummarizesArtifacts(t *testing.T) {
	w := makeTestWizardModel(fakeWriteTUIDeps(nil))
	w.inputs[0].SetValue("personal")
	w.screen = screenReview
	w.step = wizardStepForm
	w.leg1SSHPreview = "Host personal.github.com\n  Hostname ssh.github.com\n  Port 443\n"
	w.leg2Result = identity.CreateResult{
		GitconfigPreview:   "[includeIf \"gitdir:~/git/personal/\"]\n\tpath = ~/.gitconfig.d/personal\n",
		FragmentPreview:    "[user]\n\tname = Pedro Perez\n\temail = pedro@example.com\n",
		AllowedSignersLine: "pedro@example.com ssh-ed25519 AAAA personal@gitid",
	}

	view := w.view(90)
	for _, want := range []string{"Review", "Host personal.github.com", "includeIf", "path = ~/.gitconfig.d/personal", "pedro@example.com"} {
		if !strings.Contains(view, want) {
			t.Errorf("Screen 4 review must show %q; got:\n%s", want, view)
		}
	}

	// Enter closes the wizard (read-only screen).
	_, cmd := w.handleKey(tea.KeyPressMsg{Code: tea.KeyEnter})
	if cmd == nil {
		t.Error("Enter on Screen 4 must dispatch a close command")
	}
}
...
func TestWizardSigningToggle(t *testing.T) {
	...
	// Screen 3 (Git config): the signing toggle is the last Tab stop (position 3).
	w.screen = screenGitConfig
	w.focusIdx = 3
...
func TestWizardMatchSelectorNavigation(t *testing.T) {
	...
	// Screen 3 (Git config): the match selector is Tab stop position 2.
	w.screen = screenGitConfig
	w.focusIdx = 2
	w.inputs[0].SetValue("personal")
