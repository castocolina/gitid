...
// wizardLeg2ResultMsg carries the outcome of the LEG-2 (git config) write —
// identity.PersistGitconfig: the ~/.gitconfig.d/<name> fragment, the includeIf
// block in ~/.gitconfig, and the ~/.ssh/allowed_signers line. On success the
// wizard advances to Screen 4 (read-only review).
type wizardLeg2ResultMsg struct {
