...
|-----------|------------------------------|--------|--------------|--------|------------|
| 1 | (observation, not a §3 dimension) | all 7 | Same pre-existing 02-02 shell-infrastructure characteristic noted in `create-flow/CRITIQUE.md` finding #1: the TUI shell header renders only the app name + breadcrumb, not the HTML `Header.tsx`'s "N identities · ✓ healthy" context chip. Not introduced by git-screen, not fixable within this surface's own files, and not one of §3's seven MUST-match dimensions. | **FIXED** (02-review-fixes, finding A2) | Logged for awareness; not a `parity.json` row for the same reason `create-flow`'s finding #1 was not one. **Update:** the 02-review-fixes pass added the missing chip to `renderShellHeader` (shell.go) — a static "8 identities · ! needs action" fixture matching the HTML header's semantic content — rendered on every surface including git-screen (confirmed via the re-captured git-screen/tui/*.png set). Same shared shell.go fix `create-flow/CRITIQUE.md` finding #1 now documents. |
...
| 3 | labels-and-helper-copy-verbatim | `git-form-empty`, `git-form-filled` | 02-review-fixes finding A4 (external cross-vendor review, agent-ui-ux-designer parity critique): `user.signingkey`'s HTML helper text is "A PATH to the public key — never the key material itself." (git-form-empty.route.tsx / git-form-filled.route.tsx); `renderGitFormEmpty` dropped the "never the key material itself" safety clause, and `renderGitFormFilled` carried no helper text for that field at all. | **FIXED** | `surface_gitscreen.go`'s `renderGitFormEmpty`/`renderGitFormFilled` now carry the safety clause verbatim on both screens. Re-verified against the route files' literal `helperText` strings and the re-captured `git-form-empty.png`/`git-form-filled.png` html/tui pairs. See `parity.json`'s `labels-and-helper-copy-verbatim` row for the full before/after text. |
