...
| Option row height | exactly **2 lines** per option on the master list (`optionRowLines`, shared constant with global-ssh): line 1 = selection marker + checkbox glyph + tone glyph + key name + optional `[main vs master]` chip; line 2 = indented `now: <current> → <recommended>` | `view()`, `globalgit.go:322-351` | existing, unchanged — row height itself does NOT change. **RESOLVED (was OPEN layout question):** with 11(+1) rows at 2 lines/row the master list becomes a **scrolling window** over the row set instead of trying to render all rows at once — see UI Considerations "overflow" row below for the full mechanism (new `listWindowStart` field, per-row analog of `ExactTextViewport`'s scroll, reusing the "+N more" cue vocabulary from `renderReceiptList`) |
...
option rows is this phase's single highest-risk visual surface — **RESOLVED**
via the master-list scrolling window decided in UI Considerations
("overflow" row) below — everything else is absorbed inside the existing
...
|----------|------------|--------|---------------------|
| overflow | **11(+1) option rows in the master list vs global-ssh's 6** | ✓ resolved | at 2 lines/row, 12 rows = 24 body lines, close to or exceeding the measured ~24-of-25-row body budget (`02-STYLE-SPEC.md`'s corrected measurement, 02-15). **Decision: the master list becomes a scrolling window over the row set**, using this codebase's EXISTING "+N more" cue vocabulary (`renderReceiptList`, `ceremony.go:305-322`) applied per ROW instead of per line, plus the EXISTING viewport-scroll interaction model (`ExactTextViewport.ScrollDown`/`ScrollUp`, `frame.go:693-703`) as the pattern reference — never a wholly novel mechanism. Row height stays **UNCHANGED at 2 lines/row**; only which rows are visible changes. Concretely: `globalGitModel` gains a `listWindowStart int` field — the index of the first visible ROW (0-based), the per-row analog of `ExactTextViewport.LineOffset`. `visibleRowCount` is computed from the SAME `frameBodyRows(30)` helper the row-height row cites, minus the master list's existing chrome (findings banner when present). If `len(options)*optionRowLines` fits inside that budget, every row renders, `listWindowStart` stays `0`, and NO cue renders — behavior is identical to today's un-scrolled global-ssh case, zero regression. When it does NOT fit: reserve exactly one line for a cue and set `visibleRowCount = (available-1) / optionRowLines`. `↑`/`↓` move the selection cursor as today; when the newly selected row falls outside `[listWindowStart, listWindowStart+visibleRowCount)`, `listWindowStart` shifts by exactly **one row** in the scroll direction — never jumps — mirroring `ExactTextViewport.ScrollDown(1)`/`ScrollUp(1)`'s existing one-step scroll UX, just re-scoped from text lines to whole 2-line option rows. Cue rendering: when rows are hidden below the window (`listWindowStart+visibleRowCount < len(options)`), the reserved line renders `styleFaint.Render(fmt.Sprintf("↓ (+%d more options)", hidden))` directly below the last visible row — byte-identical style/format convention to `renderReceiptList`'s `"… (+%d more lines)"` cue, reworded for rows. When rows are hidden above instead (`listWindowStart > 0`) the SAME reserved line renders `"↑ (+%d more options)"` above the first visible row. Tie-break rule for the case both edges hide rows (mid-scroll): the reserved line always shows the **below** cue (`↓`) — the down direction is this list's canonical read/apply order, and "you have scrolled" is already implied by the cursor's mid-list position, so no dedicated top cue competes for the one reserved line in that state. **This is a genuinely NEW capability** — no prior master-detail screen in this codebase (Identity Manager, Global SSH) needed to scroll its list — but the mechanism is 100% composed of this codebase's existing cue vocabulary and existing viewport-scroll interaction model, not invented from scratch. |
...
| partial | D9 becomes TWO fields (D-04) — name-only, email-only, both-empty, both-set are 4 distinct states | ✓ resolved | **Decision: pressing `a` always applies exactly the currently-valid, non-empty field(s) present on the D9 detail pane, in ONE ceremony instance.** If only `user.name` is filled, only `user.name` applies. If only `user.email` is filled (and valid — `needs @` still gates), only `user.email` applies. If both are filled, both apply together in that same ceremony instance. An empty field is simply NOT included in that apply — it is neither written nor cleared by omission (it was already unset; D-04's "empty means unset" semantics already cover the write side). This is the direct, literal render of D-04's "independent per-field apply, empty=unset" semantics — there is no separate per-field selection step, no checkbox (D-04 already removed the checkbox), and no ambiguity about "whatever is currently filled": that IS the rule. Both-empty is the guarded no-op state (existing `emailValid()`-style guard on `a`, extended to "at least one of the two fields is non-empty and any non-empty field is valid" before the key opens the ceremony at all — mirrors the existing `len(m.gitApplyChosen(options)) > 0` guard pattern on the baseline ceremony). This is also what makes D-07's cross-warning knowable: at the moment `a` (or the useConfigOnly toggle) is evaluated, the missing half is exactly "the field that is empty right now," never ambiguous. |
...
| loading | D-03's provenance three-probe diff (`git config --show-origin --show-scope --list` × non-repo cwd + `--file … --list`) | ✓ resolved | **Decision: NO dedicated loading state.** The real backend's provenance-probe read happens SYNCHRONOUSLY inside `activate()`, before first render — the SAME pattern every other Global-* tab in this codebase already uses. Precedent: `globalSSHModel.activate` (`globalssh.go:136-147`) calls `m.backend.GlobalSSHOptionStates()` and `m.refetchStoragePlan()` (which itself calls `m.backend.SSHStorageMigrationPlan(...)`, `globalssh.go:161-171`) directly inline, storing the result on `m` and returning `(m, nil)` — no `tea.Cmd`, no async message round-trip, no spinner. Phase 7 replaces `globalgit.go:58`'s current no-op stub (`func (m globalGitModel) activate(DemoState) (screenModel, tea.Cmd) { return m, nil }`, dummy-fixture placeholder) with the SAME shape: `options, err := m.backend.GlobalGitOptionStates()` (the real Phase 7 backend call, D-03's probe) runs inline in `activate()`, storing `m.options`/`m.optionsErr` before the function returns. No "reading your configuration…" interstitial, no spinner glyph, no new `tea.Msg` — this phase introduces zero new async plumbing for the initial read. |
| error | git probe failure (missing `git` binary, malformed repo state at probe cwd) | ✓ resolved | **Decision: reuse Phase 6's `optionsErr` pattern EXACTLY, byte-for-byte on the render side.** `globalGitModel` gains an `optionsErr string` field (same name, same role as `globalssh.go:90`'s `optionsErr` — "carries the fetch failure the pane renders instead of a blank body"), set in `activate()`: `m.options = nil; m.optionsErr = err.Error()` when the D-03 probe fails. The render site copies `globalssh.go:872-876` verbatim: `if m.optionsErr != "" { body = ... + styleWarning.Render("! "+m.optionsErr) + "\n\n " + styleFaint.Render("<advisory sentence>") }` — an inline `!` warning-styled note rendered IN PLACE OF the option rows, never a hollow blank pane. Exact Phase 7 copy: the probe layer wraps the underlying error as `fmt.Errorf("git probe failed: %w", err)` before it reaches `m.optionsErr`, so the rendered line reads `! git probe failed: <underlying error>` (mirrors `06-UI-SPEC.md`'s citation of this exact render call site, just Git-flavored wording instead of SSH's). The advisory sentence below it: `The option states could not be read from this machine.` (byte-identical to `globalssh.go:876`'s own second line — no new phrasing style invented). This is advisory/fail-open, same as every other gap on this screen: it never blocks navigation (`←→`/`↑↓` between screens stays live), it only means the options pane cannot show rows this session. |
...
- `options-list` — primary focus: this is the FIRST master-detail screen in
  the project to carry more than 6-8 rows; the master-list scrolling window
  (RESOLVED in UI Considerations "overflow" row above — `listWindowStart`,
  per-row "+N more" cue, one-row-at-a-time scroll) is the single
  highest-risk visual surface this phase carries and the top implementation
  priority, higher than any individual copy correction — it is decided, not
  open, but it is still the first thing to verify visually at PTY e2e time.
