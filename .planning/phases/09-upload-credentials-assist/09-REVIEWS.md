...
## Cycle 2 findings — resolution (2026-08-28)

Cycle 2 (codex + xai-grok, `CYCLE_SUMMARY: current_high=3 current_actionable=7`) re-reviewed the plans post-cycle-1-revision and confirmed all 8 of cycle 1's findings were genuinely, concretely resolved (not just claimed in prose). It then surfaced 3 new HIGH concerns and 7 actionable concerns from deeper scrutiny. Given the time already invested across multiple 20-40 minute planner/checker/review cycles spanning several hours, these were resolved via **direct plan edits by the orchestrating session** rather than a third full planner-dispatch cycle — each is addressed below with its resolution and, where a fix was made, the commit it landed in (`77cf914`).

### HIGH concerns

1. **`AuthCheck` receiving a non-canonical host (codex, HIGH).** The wizard's raw SSH host was passed straight to `gh auth status --hostname`, but `gh`/`glab` track authentication per canonical web domain, never per SSH endpoint — and this project's own alt-SSH recipe convention (port 443, `ssh.github.com`) makes that mismatch the common case, not an edge case, silently downgrading autonomous upload to manual for most real identities. **RESOLVED**: `ProviderForHostname` now returns `(provider, canonicalHost)`; `AuthCheck`'s parameter is renamed `canonicalHost` at every call site; `TestAuthCheckAlwaysReceivesCanonicalHost` proves the canonical value reaches the real `auth status` argv end-to-end (09-02-PLAN.md).
2. **`SandboxHome` hiding the real `gh` OAuth session (codex, HIGH).** `gh`'s config (including the authenticated token) defaults to `$HOME/.config/gh`, so sandboxing `HOME` for the real-account test would hide the real, already-authenticated session — the opposite of the test's purpose. **RESOLVED**: explicit `GH_CONFIG_DIR`/`XDG_CONFIG_HOME` resolution before the `HOME` swap, passed through to every child process the test spawns, with `TestRealAccountResolvesRealGHConfigDir` (09-08-PLAN.md).
3. **Phase P not proving the autonomous create-time trigger (codex: HIGH, xai-grok: MEDIUM).** Phase P uses `--no-upload` at create time then explicitly runs `register-key`, proving the manual re-registration path, not the default autonomous trigger. **ACCEPTED, documented — not a safety gap.** The autonomous trigger's decision logic (checkbox derivation, create-time dispatch, announce-and-do sequencing) is exhaustively covered elsewhere in this phase (waves 2, 4, 7) against fake `gh`/`glab` shims, where every branch is independently controllable. A real account adds no additional confidence to that logic — it only proves what a shim cannot (provider acceptance of the registration and title format), which is exactly what Phase P and Phase N each test. Running the full autonomous path against a real account was considered and rejected: it would remove the create/register observability boundary Phase P relies on, for coverage this suite already has elsewhere. The human checkpoint text now states this scope boundary explicitly and names the alternative considered (09-08-PLAN.md).

### Actionable non-HIGH concerns

4. **`e2eEnv` doesn't validate caller-supplied PATH prefixes (MEDIUM).** **RESOLVED**: `e2eEnv` now rejects (via `t.Fatalf`) any prefix that is the ambient PATH or contains one of its real entries, with `TestE2EEnvRejectsAmbientPathAsPrefix` (09-02-PLAN.md).
5. **Final-sweep-always-runs guarantee not established for early/`t.Fatal` failures (MEDIUM).** **RESOLVED** — and this fix itself needed a follow-up correction, caught by a same-session sanity-check pass: the initial fix had Go's LIFO `t.Cleanup` order backwards (registering the sweep second would make it run FIRST, before cleanup, not after). Corrected: the sweep registers FIRST (step 2, before any key material exists), the outstanding-ID cleanup registers SECOND (step 8), so LIFO makes cleanup run before the sweep observes a clean state, with `TestRealAccountCleanupOrderingIsLIFOSweepLast` proving the order directly (09-08-PLAN.md).
6. **09-02's `files_modified` list incomplete for `newRealCreateFlowCmd` callers (MEDIUM).** **RESOLVED (clarified)**: `e2e/global_ssh_storage_pty_e2e_test.go` needs no file change — it calls the shared `newRealCreateFlowCmd` helper rather than constructing its own environment, so it inherits hermeticity automatically once the helper is migrated. `e2e/install_e2e_test.go` was already a documented allowlist exception (`make install`, never runs the product). The plan text now states both explicitly so the absence from `files_modified` doesn't read as a gap (09-02-PLAN.md).
7. **`--dry-run` must_haves text overclaims "executes no provider command at all" (LOW).** **RESOLVED**: reworded to match the plan's own R11 reasoning — no remote mutation, but auth/inventory reads are allowed (09-05-PLAN.md).
8. **Deny-shim state described as `Disabled` when it's actually `Unauth` (LOW).** No plan-text defect — this was guidance for what the EXECUTOR's future SUMMARY.md should not misclaim, not a claim the plan itself makes. No edit needed.
9. **`TestEveryE2EChildEnvIsHermetic` not pinned to AST-level detection (LOW).** **RESOLVED**: specified to use `go/parser`+`go/ast` walking `*ast.AssignStmt`/`.Env` selectors, explicitly ruling out a `grep`/string-presence approach (09-02-PLAN.md).
10. **09-02's size/risk (LOW).** Already mitigated by the existing mandated two-commit split (Part A = the PATH-safety boundary alone, landable independently); no further action taken given the explicit stop-at-boundary contingency already in the plan.

### Also caught in this pass (not from either review cycle)

- Duplicate threat-model ID `T-09-02-08` used for two distinct rows — renumbered the second to `T-09-02-09`.

### Disposition

No third cross-AI review cycle was run. Given (a) cycle 1's findings were independently confirmed resolved by cycle 2's reviewers reading the actual current plan files against the live codebase, not commit-message prose, and (b) every cycle-2 finding above has a concrete resolution — either a plan-text fix with a named test, or an honestly-documented and reasoned scope decision — the phase proceeds to execution. Any residual risk is caught by the identical independent re-verification discipline (build/vet/`test -race`/lint/`gate-visual-regression`/`test-e2e`, personally re-run, never trusting an executor's self-report) already established across every prior phase in this project, applied specifically to Wave 2 (the PATH-safety and canonical-host fixes) and Wave 8 (the GH_CONFIG_DIR and cleanup-ordering fixes) before either is considered done.
