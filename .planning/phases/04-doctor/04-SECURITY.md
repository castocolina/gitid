...
| T-04-02 | Tampering | mitigate | CLOSED | checks/perms.go:15-19 — KEY-02 constants (0700/0600/0644/0600/0644); checkPath captures `p, m := path, want` and calls `deps.FixPerm(p, m)`; 0644 is the only non-restrictive target (pub key); perms.go:94 |
...
| T-04-19 | Tampering | mitigate | CLOSED | cmd/gitid/doctor.go:271-275 — WR-02: `mode := os.FileMode(0o600)` then `if path == allowedSignersPath { mode = 0o644 }`. Only two values possible: 0600 or 0644. checks/perms.go:83-94 — FixPerm receives KEY-02 constants only (checkPath `want` param is always a KEY-02 constant) |
