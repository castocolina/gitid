...
Status: Wave 5 of 8 COMPLETE and merged; starting Wave 6 (08-06-PLAN.md)
Last activity: 2026-08-28 — Wave 5 (08-05) hand-implemented directly after the cross-AI executor crashed almost immediately (a permission auto-reject on an external directory path after a typo'd `cd`, zero code committed, still in investigation phase). Delivered the remaining D-05 Coherence-family rows, every one wrapping an existing, tested Phase 6/7 probe unchanged — no shadow-detection or author-resolution logic reimplemented: `checkShadowedGlobalOptions` reuses `internal/globalssh.Verify` (the same D-04 post-write probe `runGlobalSSHApply` already runs), scoped to the non-per-alias policy keys gitid has actually written into its global `Host *` block; `checkDirectiveAboveManagedBlock` reuses `deps.AllHostBlocks`/`ParseAllHostBlocks` to flag a hand-written Host stanza preceding gitid's first managed block (D-09); `checkAuthorResolution` reuses `internal/globalgit.VerifyAuthorResolution` (the same D-06 probe `appendFallbackAuthorAdvisories` already runs) per managed identity. Also resolved 08-RESEARCH.md's Pitfall 6 (the includeIf-missing-fragment Family conflict): read `orphans.go`'s Class 2 against `coherence.go`'s Incomplete branch side by side — fragment-existence detection lives entirely in Coherence, so D-05's "Coherence / Git" label is the correct, resolved answer (the frozen dummy fixture's "Orphans" tag is a stale artifact); added a dedup guard to `orphans.go`'s Class 2 loop (skip identities Coherence already reported Incomplete) and `TestMissingFragmentNoDuplicate` proving exactly one finding via the real `doctor.Run()` path. All three new checks are report-only (`Fix: nil`); verified via real fixtures (a fresh empty home produces no false alarms; a hand-built shadowed/directive-above config correctly triggers both new checks, cross-checked directly against `ssh -G`/`ssh -F <file> -G`) — no apply-fix-then-rescan convergence check applies since none of this wave's findings carry a Fix. Merged (fast-forward) to `gsd/phase-08-health-fixer` at `620e45a`; full gate battery (build, `go test -race` 2134 passed — two independent runs, lint 0 issues, `gate-visual-regression` 74.6s, `make test` incl. `gate-copy-freeze`, `make test-e2e` 604.6s) independently re-verified green on the merged tree; several 06-global-ssh-options ui-frame fixtures legitimately gained new finding counts the new checks correctly surface (kept in full); pure re-run noise (backup timestamps, ephemeral `t.TempDir()` paths) reverted. See `08-05-SUMMARY.md`. 3 waves remain (08-06 through 08-08); see `08-REVIEWS.md` for the cross-AI review these plans already incorporate.
